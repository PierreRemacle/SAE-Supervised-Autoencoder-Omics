...
if __name__ == "__main__":
    ...
    lambda_wass = 0.5  # Regularization parameter
    ...
    # file_name = 'Synth_1000f_16inf_1000s.csv'
    # file_name = 'dataset_anonymized.csv'
    file_name = './TimeInterval.csv'

    test_size = 0.5  # Test size during the train test split
    ...
    ETA = 2.0  # Controls feature selection (projection) (L1, L11, L21)
    ...
    seed_idx = 0
    data_train = np.zeros((nfolds * len(SEEDS), 8))
    data_test = np.zeros((nfolds * len(SEEDS), 8))
    data_retrain = np.zeros((nfolds * len(SEEDS), 8))
    data_retest = np.zeros((nfolds * len(SEEDS), 8))

    data_transfert_Test = np.zeros((nfolds * len(SEEDS), 8))

    sparsity_matrix = np.zeros((nfolds * len(SEEDS), 1))
    sparsity_matrix_retraing = np.zeros((nfolds * len(SEEDS), 1))
    for seed in SEEDS:
        brier_scores_fold = None
        ...
        for fold_idx in range(nfolds):
            ...
            if seed == SEEDS[-1]:
                if fold_idx == 0:
                    ...
                    LP_train = data_encoder
                else:
                    # Aggregate results across folds
                    integrated_brier_scores.append(integrated_brier_score)
                ...
                print(f"Mean Integrated Brier Score: {mean_brier_score:.4f}")
                ...
                train_hazards = LP_train[:, 0]
                train_durations = LP_train[:, 1]  # True durations
                train_events = LP_train[:, 2]  # Event indicators
                ...
                concordance_index = concordance_index_censored(
                    np.array(events, dtype=bool), durations, hazards)
                train_concordance_index = concordance_index_censored(
                    np.array(train_events, dtype=bool), train_durations, train_hazards)
                print(f"Concordance Index: {concordance_index}")
                plt.figure()
                plt.plot(survival_times, brier_scores, label="Brier Score")
                plt.xlabel("Time")
                ...
                if (fold_idx == 0):
                    brier_scores_folds = brier_scores
                else:
                    brier_scores_folds += brier_scores
            ...
            data_test[seed_idx * 4 + fold_idx, 5] = wasserstein_distance(labels_encodertest / np.sum(
                labels_encodertest), label_predicted_test / np.sum(label_predicted_test)) * divided

            data_train[seed_idx * 4 + fold_idx,
                       6] = integrated_brier_score  # incorect
            # i dont beleive it make sense to compute the brier score on the training set.
            # Brier score is computed by comparing the predicted survival function to the true survival function.

            data_test[seed_idx * 4 + fold_idx, 6] = brier_scores.mean()

            data_train[seed_idx * 4 + fold_idx, 7] = train_concordance_index[0]

            data_test[seed_idx * 4 + fold_idx, 7] = concordance_index[0]
            ...
            if DoSparsity:
                ...
                mat_in = net.state_dict()["net.0.linear.weight"]
            ...
            data_transfert_Test[seed_idx * 4 + fold_idx, 0] = metrics.mean_squared_error(
                label_predicted_test20, labels_encodertest20
            )

            data_transfert_Test[seed_idx * 4 + fold_idx, 1] = metrics.mean_squared_error(
                label_predicted_test20, labels_encodertest20
            )**0.5 * divided

            # MAE score
            data_transfert_Test[seed_idx * 4 + fold_idx, 2] = metrics.mean_absolute_error(
                label_predicted_test20, labels_encodertest20
            ) * divided

            data_transfert_Test[seed_idx * 4 + fold_idx, 3], data_transfert_Test[seed_idx * 4 + fold_idx, 4] = ft.valueGap(
                label_predicted_test20, labels_encodertest20, divided
            )

            data_transfert_Test[seed_idx * 4 + fold_idx, 5] = wasserstein_distance(
                labels_encodertest20 /
                np.sum(labels_encodertest20), label_predicted_test20 /
                np.sum(label_predicted_test20),
            ) * divided

            data_transfert_Test[seed_idx * 4 +
                                fold_idx, 6] = brier_scores.mean()
            data_transfert_Test[seed_idx * 4 +
                                fold_idx, 7] = concordance_index[0]
    ...
    reg_metrics = ["MSE", "RMSE", "MAE", "Negative gap",
                   "Positive gap", "WD", "brier score", "Concordance Index"]
    df_metricsTrain, df_metricsTest = ft.packMetricsResult(
        data_train, data_test, nfolds * len(SEEDS), reg_metrics
    )
    ...
    df_metricsTransfertTest = ft.packMetric(
        data_transfert_Test, nfolds*len(SEEDS), reg_metrics)
    df_metrics_TransfertTest = df_metricsTransfertTest[reg_metrics]
    print("\nMetrics Transfert Test")
    # print(df_metricsTrain_clustering)
    print(df_metrics_TransfertTest)
    ...
    if DoSparsity:
        ...
        print(df_sparcity)
        # print(f'\n On average we have {round(100-df_sparcity.loc["Mean", "Sparsity"])}% features selected')
        # print(f'thus {round(((100 - df_sparcity.loc["Mean", "Sparsity"])/100)*feature_len)} features')
    ...
    print(f"Execution time: {execution_time_global} seconds")
